...
const ops=require('../v5/ops-api').createOps({env,store}),chat=require('../v5/cloud-chat').createCloudChat({env,store,fetcher:async(url,options)=>{
 if(url!=='https://api.deepseek.com/chat/completions')throw Error('UNEXPECTED_TEST_UPSTREAM');
 modelCalls++;
 const payload=JSON.parse(JSON.parse(options.body).messages.at(-1).content);
 // Use only IDs actually supplied by retrieval. The production citation gate stays intact.
 const source_ids=(payload.evidence||[]).slice(0,1).map(row=>row.id);
 return new Response(JSON.stringify({choices:[{finish_reason:'stop',message:{content:JSON.stringify({intent:{kind:'other',responseMode:'general',city:'Shanghai'},text:'I can help plan a relaxed visit with your parents. Tell me which neighborhood you prefer and I will help compare your options.',source_ids})}}],usage:{prompt_tokens:30,completion_tokens:10,total_tokens:40}}));
}});
const root=path.resolve(__dirname,'../dist');
http.createServer((req,res)=>{const p=new URL(req.url,'http://local').pathname;
 if(p==='/__test/status'){res.writeHead(200,{'Content-Type':'application/json','Cache-Control':'no-store'});return res.end(JSON.stringify({fixture:true,modelCalls}));}
