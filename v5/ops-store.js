...
function createStore(env=process.env,sdkOverride){
 ...
 return{kind:'local-sqlite',read:async()=>read(),async mutate(fn){db.exec('BEGIN IMMEDIATE');try{const value=read()||require('./operations').initial(),result=fn(value);value.revision++;db.prepare('INSERT INTO state(id,value) VALUES(1,?) ON CONFLICT(id) DO UPDATE SET value=excluded.value').run(JSON.stringify(value));db.exec('COMMIT');return result;}catch(e){db.exec('ROLLBACK');throw e;}},close:()=>db.close()};
}
