...
try:
 with sync_playwright() as p:
  ...
  expect(page.locator('.offer-card .route').first).to_contain_text('上海虹桥 → 杭州东')
  check('rail reference is automatically attached',page.locator('[data-source-detail="sh-hz-rail"]').count()>0)
  page.locator('.offer-link').first.scroll_into_view_if_needed()
  with context.expect_page() as popup:page.locator('.offer-link').first.click()
  popup.value.close()
  ...
  page.locator('#nav-ops').click();expect(page.locator('#ops-cloud-login')).to_be_visible();page.screenshot(path=str(OUT/'V5_7_手机运营登录.png'))
