'use strict';
const L=require('./library'),E=require('./engine'),I=require('./intent-tools');
...
async function assist({state,model,signal,records=L.records}){
 ...
 if(required.some(id=>records.some(r=>r.id===id&&!L.current(r))))return{text:state.language==='zh'?'本次问题所需资料已过复核期或原文有变化，相关事实暂不用于回答。请到资料库查看复核状态，或直接查询官方与服务商入口。':'A required source is due for review or has changed. I will hold the related factual claims; check its review status or consult the official/provider link.',sourceIds:[],mode:'source-gap',notice:'required-source-held'};
