...
const Request=require('./request-semantics'),Arbitration=require('./answer-arbitration'),Cities=require('./city-scope'),KB=require('./kb-direct'),Queries=require('./retrieval-plan'),Guard=require('./answer-guard');
const numbers=value=>(String(value||'').match(/\d+(?:[.,]\d+)*\s*%?/g)||[]).map(n=>n.trim().replace(/\s+/g,''));
const numberEvidence=(evidence,history)=>({sources:evidence.map(e=>({sourceId:e.id,numbers:[...new Set(numbers([e.summary,e.reviewedAt,e.published].filter(Boolean).join(' ')))]})),traveler:[...new Set(numbers(history.map(h=>h.text).join(' ')))]});
...
function validate(value,evidence,history){
 ...
 const unsupported=[...new Set(numbers(claims).filter(n=>!supportedNumbers.has(n)))];if(unsupported.length){const error=Error('ANSWER_NUMBER');error.unsupportedNumbers=unsupported;throw error;}
...
async function assist({state,model,signal,records=L.records,skillConfig=H.DEFAULT,acquire=null}){
 ...
 const directStarted=Date.now(),direct=KB.lookup(h.text,{city:state.facts.city,language:state.language,records});
 if(direct.matched)return{...direct.answer,execution:{skillVersion:skillConfig.version||H.DEFAULT.version,totalMs:Date.now()-directStarted,kb:direct.trace,stages:[{name:'kb_direct',status:'completed'},{name:'verify',status:'completed'},{name:'observe',status:'pending-persistence'}]}};
 if(model.status?.().configured){const started=Date.now(),execution={skillVersion:skillConfig.version||H.DEFAULT.version,stages:[]};const answer=await smartAssist({state,model,signal,records,skillConfig,execution,acquire});execution.kb={matched:false,reason:direct.reason};execution.totalMs=Date.now()-started;execution.stages.push({name:'verify',status:answer.mode==='source-gap'?'held':'completed'},{name:'observe',status:'pending-persistence'});return{...answer,execution};}
 const active=new Set(records.filter(r=>r.active!==false).map(r=>r.id));
 const city=state.facts.city,tool=I.analyze(h.text,city,state.language,state.history.slice(0,-1)),lookup=tool.sourceIds.length&&tool.sourceIds.every(id=>records.some(r=>r.id===id))&&!h.sourceIds?.length?{...h,sourceIds:tool.sourceIds}:h,selected=L.choose(lookup,city,records),compatible=selected.filter(r=>active.has(r.id)&&Cities.applies(r,city));
...
async function smartAssist({state,model,signal,records,skillConfig=H.DEFAULT,execution={stages:[]},interpreted=null,acquire=null}){
 ...
 if(validationIssue){
  ...
  const generationPrompt=PROMPT+MI.languageRule(state.language)+' Use plain paragraphs or unnumbered bullets. Include only numbers present in the cited evidence or provided in the traveler request; omit unknown prices, sizes, distances and opening hours. The numberEvidence map is a spelling aid, not permission to attach a number to another fact. Even suggested time buffers must not introduce unsupported numbers; say leave enough time instead.';
  const generationPayload={language:state.language,currentRequest:h.text,intent,validationIssue,history:state.history.slice(-6).map(x=>({traveler:x.text,companion:x.assistance?.text||x.reply})),evidence:applicable,numberEvidence:numberEvidence(applicable,state.history),advisoryStyle:skillConfig.guidance};
  const started=Date.now(),draft=await model.call(generationPrompt,generationPayload,signal,{temperature:0});generated=draft.value;
  ...
  try{answer=checked(generated);}catch(error){
   // Repair a rejected numeric claim once, against exactly the same evidence.
   // This also covers a valid intent draft followed by a numeric failure during
   // grounded generation; repeated fabrication remains blocked.
   if(error.message!=='ANSWER_NUMBER'){error.modelUsage=meta.usage;throw error;}
   signal?.throwIfAborted();const repairStart=Date.now(),repaired=await model.call(generationPrompt+' The preceding draft failed numeric validation. Remove unsupported numeric claims or replace them with non-numeric advice. Do not change their spelling to Chinese numerals to bypass this check. Return a complete answer with the same source constraints.',{...generationPayload,validationIssue:error.message,unsupportedNumbers:error.unsupportedNumbers,previousDraft:generated},signal,{temperature:0});
   meta.usage=Object.fromEntries(['prompt_tokens','completion_tokens','total_tokens'].map(k=>[k,(meta.usage?.[k]||0)+(repaired.usage?.[k]||0)]));
   execution.stages.push({name:'numeric_repair',status:'completed',attempts:1,ms:Date.now()-repairStart});
   try{answer=checked(repaired.value);}catch(repeated){repeated.modelUsage=meta.usage;throw repeated;}
  }
 }
 ...
 const local=I.analyze(h.text,intent.city,state.language,prior);const eligibleLocal=local.kind===intent.kind&&local.text&&local.sourceIds?.length&&local.sourceIds.every(id=>records.some(r=>r.id===id&&L.current(r)))&&!['guide','service'].includes(intent.responseMode)?{text:local.text,sourceIds:local.sourceIds}:null;const guarded=Guard.select(eligibleLocal,answer,intent,h.text,records,Arbitration);execution.answerGuard=guarded.guard;if(guarded.held)return{...meta,mode:'source-gap',sourceIds:[],text:zh?'当前候选缺少有效且适用的依据，暂不采用。请查看原文或补充具体服务渠道。':'The candidates lack applicable current evidence; check the source or clarify the service channel.',confidence:{requiresReview:true,reasons:['candidate-eligibility']}};const arbitration=guarded.decision;execution.arbitration=arbitration;answer={...answer,...guarded.answer};
