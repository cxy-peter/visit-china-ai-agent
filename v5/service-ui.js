...
function answerHTML(h){if(h.assistance?.tasks?.length>1)return "";
 ...
 return (window.TravelTransportUI?.html(h)||'')+(window.TravelThemeUI?.html(h)||'')+(a?.modelRouting?'<small class="answer-model">'+esc(a.modelRouting.selected.toUpperCase())+' · '+esc(a.modelRouting.reason)+'</small>':'')+(body?'<div class="answer-services">'+body+'</div><p class="muted">'+(zh?'可在运营方查询当前价格和可用量；未下单或预订。':'Check providers for current prices and availability; no order or reservation has been made.')+'</p>':'')+(window.TravelRecommendationUI?.products(h)||'')+(a?.arbitration?'<details class="answer-arbitration"><summary>'+(zh?'回答选择依据':'Answer selection')+'</summary><p>'+esc(a.arbitration.reason)+'</p><small>'+esc(a.arbitration.selected)+' · '+(a.arbitration.local?'工具 '+a.arbitration.local.score+' / ':'')+(a.arbitration.deepseek?'DeepSeek '+a.arbitration.deepseek.score:'')+' · '+esc(a.arbitration.scope)+'</small></details>':'')+(a?.execution?'<details class="answer-evidence"><summary>'+(zh?'这次回答查了哪些资料？':'What evidence was retrieved?')+'</summary><ul>'+evidence.map(r=>{const url=webUrl(r.url);return url?'<li><a href="'+esc(url)+'" target="_blank" rel="noopener noreferrer">'+esc(r.title)+' ↗</a></li>':'';}).join('')+'</ul></details>':'');
}
