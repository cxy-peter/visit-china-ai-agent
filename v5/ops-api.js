...
function catalog(s){const rows=[...s.sources,...require('./discovery').records().filter(r=>!s.sources.some(x=>x.id===r.id))];return rows.map(r=>{const check=s.checks.find(c=>c.id===r.id);return{...r,lastCheck:check?{at:check.at,status:check.status,error:check.error}:null,active:r.active!==false&&!(check?.status==='changed'&&check.baseSourceHash===G.digest(r))};});}
function createOps({env=process.env,store=createStore(env),fetcher=fetch}={}){
 ...
 return async(req,res)=>{try{const url=new URL(req.url,'https://local'),isCron=url.pathname.endsWith('/source-refresh');
  ...
  if(b.action==='evaluate-scenarios'){if(!admin)throw Error('ADMIN_REQUIRED');const run=await require('./scenario-eval').evaluate({modelConsent:b.modelConsent===true,env,fetcher});await store.mutate(s=>{s.runs.push({...run,checks:run.checks.filter(c=>!c.passed).concat(run.checks.filter(c=>c.passed).slice(0,40)),displayScope:'全部失败（如有）与前40条通过样例；完整题目见测试集下载'});s.runs=s.runs.slice(-5);O.log(s,'scenario_evaluation',actor.name,{id:run.id,passed:run.passed,total:run.total});});return json(res,200,{run});}
