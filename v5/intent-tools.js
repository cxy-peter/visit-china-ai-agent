...
function analyze(text,city,language='zh',history=[]){
 ...
 const metro=M.intent(t,city);if(metro)return{kind:'metro',sourceIds:['sh-metro-map'],metro,text:metro.origin?tr('从'+M.origins[metro.origin].zh+'到豫园：'+M.route(metro.origin).line+'号线，'+M.route(metro.origin).direction+'。到豫园站后按导向步行至景区，出口开放以现场为准。','From '+M.origins[metro.origin].en+' to Yu Garden: Line '+M.route(metro.origin).line+', '+M.route(metro.origin,'en').direction+'. Follow signs from the station to the attraction; check open exits on site.'):tr('告诉我你的出发地铁站，我会显示从哪里到哪里、乘坐线路和最后一段步行。','Tell me your departure station for a route diagram and the final walking leg.')};
