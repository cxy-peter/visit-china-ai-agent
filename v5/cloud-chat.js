...
function createCloudChat({env=process.env,fetcher=fetch,now=()=>Date.now()}={}){
 ...
 return async(req,res)=>{
  try{
   ...
   if(b.action==='login'||b.action==='admin-connect'){
    ...
    else {const supplied=require('./access-code').parse(b.accessCode);if(!supplied||!equal(supplied,access.trim()))return json(res,401,{error:'ACCESS_CODE_INVALID'});}
   ...
   try{let records=require('./library').records,skillConfig=require('./harness').DEFAULT;if(env.BLOB_STORE_ID||env.BLOB_READ_WRITE_TOKEN||env.CLOUD_OPERATIONS==='1'){const stored=await require('./ops-store').createStore(env).read()||require('./operations').initial();records=require('./ops-api').catalog(stored);skillConfig=require('./harness').state(stored).active;const version=b.policyVersion;const published=stored.governance.candidates.find(c=>c.publishedVersion===version&&c.status==='published');const previous=stored.governance.candidates.find(c=>c.previous?.version===version)?.previous;state.policy=published?{...published.config,version}:previous||stored.governance.active;}const answer=await assist({state,model,signal:controller.signal,records,skillConfig});answer.dataset=b.evaluation===true&&identities.actor(req)?.role==='admin'?'evaluation':'live';if(answer.confidence&&(env.BLOB_STORE_ID||env.BLOB_READ_WRITE_TOKEN||env.CLOUD_OPERATIONS==='1')){try{answer.assessmentId=await require('./ops-store').createStore(env).mutate(s=>{answer.executionId=require('./harness').observe(s,answer);return require('./operations').assessment(s,answer);});answer.confidence.reviewRecorded=true;}catch(_){answer.confidence.reviewRecorded=false;}}return json(res,200,{answer,revision:b.revision});}finally{clearTimeout(timer);}
