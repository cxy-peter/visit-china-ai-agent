...
try:
    with sync_playwright() as p:
        browser=p.chromium.launch(headless=True,**({'executable_path':os.environ.get('CHROMIUM_PATH','/usr/bin/chromium')} if static else {}))
        ...
        page.locator('[data-choice="taxi"]').click();page.locator('[data-choice="museums"]').click()
        ...
        if static:
            ...
            page.locator('[data-action="evaluate"]').click();page.wait_for_function("document.querySelector('.candidate').innerText.includes('通过')")
            ...
            for i in range(1,6):
                ...
                page.wait_for_function(f"document.querySelector('.candidate').innerText.includes('{i}/5')")
            ...
            page.wait_for_function("document.querySelector('.candidate .status').textContent==='rolled_back'")
