...
try:
    with sync_playwright() as p:
        ...
        check('browser online speech is the only selectable provider', page.locator('#voice-provider option').count()==1)
        check('paid or local voice runtimes are not loaded', page.evaluate("typeof TravelRealtime==='undefined' && typeof TravelLocalVoice==='undefined'"))
        ...
        page.evaluate('__finish()');page.wait_for_function("()=>TravelApp.getCall().phase==='listening'")
        check('automatically listens after playback',page.evaluate("TravelApp.getCall().phase==='listening'"))
        for fragment in ['啊','嗯嗯','今天的话']:
            old=page.evaluate('TravelApp.getState().revision');spoken=page.evaluate('__spoken.length');page.locator('#interrupt').click();page.evaluate('(text)=>__say(text,true)',fragment)
            check('accidental voice fragment ignored: '+fragment,page.evaluate('TravelApp.getState().revision')==old and page.evaluate('__spoken.length')==spoken and page.evaluate("TravelApp.getCall().phase==='listening'"))
        ...
        page.evaluate('__finish()');page.wait_for_function("()=>TravelApp.getCall().phase==='listening'");page.evaluate("__say('I also need to',false)")
        ...
        check('understood needs stay visible','Flight:' in page.locator('#summary').text_content())
        ...
        check('relevant extension appears','luggage' in page.locator('#extensions').text_content().lower())
        ...
        if not static:
            ...
            check('correction updates current city but preserves opening request','Beijing' in offline.locator('#summary').text_content() and offline.locator('#initial-request').text_content()==brief)
            offline.locator('#chat-settings-open').click();offline.locator('#remember').uncheck();offline.reload()
