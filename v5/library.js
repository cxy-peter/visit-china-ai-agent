...
function choose(h,city,pool=records){const map=new Map(pool.map(r=>[r.id,r])),explicit=(h?.sourceIds||[]).filter(id=>map.has(id)).slice(0,3);if(explicit.length)return explicit.map(id=>map.get(id));
 ...
 if(!/签证|入境|支付|地铁|豫园|护照|铁路|火车|高铁|机场|政策|出租车|打车|车费|visa|entry|payment|metro|passport|rail|train|airport|policy|taxi|yuyuan|yu garden/i.test(h?.text||'')&&!pool.some(r=>(r.topics||[]).some(t=>t.length>1&&(h?.text||'').toLowerCase().includes(t.toLowerCase()))))return [];
