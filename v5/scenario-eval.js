...
async function evaluate({modelConsent=false,env={},fetcher=fetch}={}){
 ...
 return{id:crypto.randomUUID(),at:new Date().toISOString(),dataset:'synthetic-v1',promptVersion:'needs-first-v1',total:checks.length,passed:checks.filter(c=>c.passed).length,checks,model,scope:'Tool/prompt regression, not weight training, live traffic, or independent semantic accuracy. No automatic publication.'};
}
