...
ROOT=pathlib.Path(__file__).resolve().parents[1];OUT=ROOT/'evidence/v6.5/compat-browser';OUT.mkdir(parents=True,exist_ok=True);checks=[];errors=[]
...
try:
 ...
 with sync_playwright() as p:
  browser=p.chromium.launch(executable_path=os.environ.get("CHROMIUM_PATH") or shutil.which("chromium"));page=browser.new_page(viewport={'width':1536,'height':1050});page.on('pageerror',lambda e:errors.append(str(e)));page.goto(URL)
  ...
  page.locator('#new-chat-top').click();check('new chat clears current scene',page.locator('.history-compare').count()==0);check('archives retained',page.locator('[data-chat-id]').count()>=3);page.route('**/api/chat',lambda route:route.fulfill(status=200,content_type='application/json',body=json.dumps({'mode':'ignored'})) if route.request.method=='POST' and route.request.post_data_json.get('action')=='answer' else route.continue_());send('无明确需求的测试片段');check('ignored response safely stays silent',page.locator('#model-progress').is_hidden() and page.evaluate('TravelApp.getState().history.at(-1).assistance===null'));check('no browser errors',not errors);browser.close()
finally:
