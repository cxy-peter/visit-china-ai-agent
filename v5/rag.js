...
function retrievePrepared(query,prepared,overrides={}){
 ...
 // Rare literal title terms (for example a payment brand) remain useful when a
 // long conversational preface contains many generic transport/location words.
 const named=d=>d.title.split(/\s*\/\s*/).some(t=>t.length>1&&query.toLowerCase().includes(t.toLowerCase()))||tokens(d.title).some(t=>/^[a-z][a-z0-9]{2,}$/.test(t)&&qset.has(t)&&(prepared.df.get(t)||0)<=Math.max(3,docs.length*.002));
 ...
 const ranked=a.map((d,i)=>{const rrf=1/(60+i+1)+1/(60+ranks.get(d.id)+1);const coverage=d.matches/Math.max(1,qset.size),named=d.named,weight=d.recordType==='station'?(named?1.4:.5):d.recordType==='place'?(named?1.3:.8):(named?1.45:1.15);return{...d,score:rrf*(.65+.35*coverage)*weight,coverage};}).sort((a,b)=>b.score-a.score);
