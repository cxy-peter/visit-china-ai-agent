...
function source(row,now=Date.now()){
 ...
 const score=held?15:!summary?30:!current?45:row.verification?.score?Math.min(95,row.verification.score):gov?90:operator?85:row.publicationHash?72:60;
 ...
 return{score,level:score>=85?'high':score>=60?'medium':'low',requiresReview:score<85,government:gov,reasons,scope:'资料证据评分；不是事实正确概率，也不保证实时适用'};
}
function answer(intent,records,tool,modelConfidence){
 ...
 return{model:structured,modelLabel:'DeepSeek 自评（未经校准，非正确率）',sources,sourceScore:score,requiresReview:!complete||(structured!==null&&structured<0.75)||(score!==null&&score<85),reasons:[...(!complete?['缺少完成本次请求所需的信息']:[]),...(structured!==null&&structured<0.75?['模型意图自评偏低']:[]),...(score!==null&&score<85?['资料证据不足或待复核']:[])],scope:'置信度辅助排查，不替代原文核对；高分不保证答案正确'};
}
