...
try:
 ...
 with sync_playwright() as p:
  ...
  page.locator('[data-rail-after]').fill('');page.locator('[data-rail-before]').fill('');page.locator('[data-rail-period]').select_option('morning')
  with page.expect_response(lambda r:r.url.endswith('/api/products')) as got:page.locator('[data-rail-prepare]').click()
  b=got.value.json();check('period-only selection is preserved in mock search',b['request']['after']=='00:00' and b['request']['before']=='11:59');page.locator('[data-extension-close]').click()
  ...
  expect(page.locator('[data-skill-revalidate]')).to_be_visible();check('latest pending candidate opens automatically without hidden controls',True);check('candidate remains unpublished',c['status'] in ['accepted','failed'] and page.locator('#skill-form [name=topK]').input_value()=='8')
