...
OUT=ROOT/('evidence/v6.1/legacy-browser/hosted' if os.environ.get('CHAT_TEST_URL') else 'evidence/v6.1/legacy-browser');OUT.mkdir(parents=True,exist_ok=True)
...
try:
    ...
    with sync_playwright() as p:
        ...
        expect(page.locator('.offer-group')).to_have_count(0)
        check('four requested service types follow the relevant assistant bubble',all(page.locator('#transcript .companion [data-metric-kind="'+kind+'"]').count()>0 for kind in ['hotel','restaurant','rail','flight']))
        check('verified service records replace only their own sample category',page.locator('.answer-services [data-metric-kind=hotel]').count()>0 and page.locator('.history-compare[data-metric-kind=flight]').count()==1 and page.locator('.offer-group[data-metric-kind=hotel],.offer-group[data-metric-kind=restaurant],.offer-group[data-metric-kind=rail]').count()==0)
        check('flight record has dated source instead of fabricated inventory','2025' in page.locator('.history-compare').inner_text())
        ...
        check('economy preference stays visible without inventing verified hotel prices',page.evaluate("TravelApp.getState().preferences.budget==='economy'") and '320' not in page.locator('.answer-services').inner_text())
        page.screenshot(path=str(IMAGES/'chat-cards-desktop.png'),full_page=True)
        ...
        page.screenshot(path=str(IMAGES/'taxi-estimate.png'))
        ...
        send('我计划去北京，需要酒店。');expect(page.locator('.offer-group[data-metric-kind=hotel]')).to_contain_text('320')
        check('economy preference still changes unsupported-city sample hotel budget','模拟展示' in page.locator('.offer-group[data-metric-kind=hotel]').inner_text())
        page.locator('#new-chat-top').click();expect(page.locator('#transcript .user')).to_have_count(0)
        ...
        page.screenshot(path=str(IMAGES/'chat-mobile.png'),full_page=True)
        ...
        page.screenshot(path=str(IMAGES/'metro-conversation-desktop.png'),full_page=True)
        page.locator('#new-chat-top').click();page.locator('[data-demo=railout]').click();page.locator('#send').click()
        check('outbound Shanghai rail guide and actual provider follow conversation',all(station in page.locator('.utterance.companion').last.inner_text() for station in ['上海虹桥','杭州东']) and page.locator('.utterance.companion').last.locator('a[data-product-kind=rail][href*="12306"]').count()>0)
        page.locator('#new-chat-top').click();page.locator('[data-demo=flight]').click();page.locator('#send').click()
        check('inbound flight uses real airport archive instead of invented fares',page.locator('.history-compare a[href*=shairport]').count()>0 and '2025' in page.locator('.history-compare').inner_text())
        ...
        page.screenshot(path=str(IMAGES/'metro-conversation-mobile.png'),full_page=True)
