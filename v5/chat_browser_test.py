...
try:
    ...
    with sync_playwright() as p:
        ...
        expect(page.locator('.offer-group')).to_have_count(1)
        ...
        check('verified service records replace only their own sample category',page.locator('.answer-services [data-metric-kind=hotel]').count()>0 and page.locator('.offer-group[data-metric-kind=flight]').count()==1 and page.locator('.offer-group[data-metric-kind=hotel],.offer-group[data-metric-kind=restaurant],.offer-group[data-metric-kind=rail]').count()==0)
        check('remaining flight example is explicitly simulated',page.locator('.offer-warning').count()==1 and '非实时' in page.locator('.offer-warning').first.inner_text())
