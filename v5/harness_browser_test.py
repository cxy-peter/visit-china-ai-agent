"""V6 local browser acceptance: real auth/store/RAG, controlled DeepSeek HTTP response."""
import os,json,pathlib,socket,tempfile,subprocess,time,urllib.request,shutil,hashlib
from playwright.sync_api import sync_playwright,expect
ROOT=pathlib.Path(__file__).resolve().parents[1];OUT=ROOT/'evidence/v6';OUT.mkdir(parents=True,exist_ok=True)
runtime=pathlib.Path(tempfile.mkdtemp(prefix='vc6-browser-'));checks=[];errors=[]
...
fixture="""const network=global.fetch;global.fetch=async(url,opts)=>{if(!String(url).includes('api.deepseek.com'))return network(url,opts);return new Response(JSON.stringify({choices:[{finish_reason:'stop',message:{content:JSON.stringify({intent:{kind:'charging',destination:'人民广场',city:'Shanghai'},confidence:.9,text:'商场、餐厅和地铁站可寻找共享充电宝租借柜。先核对费用及归还规则；可用余量请在服务商页面查询。',source_ids:['sh-power-bank']})}}],usage:{prompt_tokens:100,completion_tokens:50,total_tokens:150}}));};const app=require('./v5/server').createApp();app.server.listen(Number(process.env.PORT),'127.0.0.1');"""
...
try:
 ...
 with sync_playwright() as p:
  ...
  check('power-bank map search honestly labels inventory missing','地图／平台查询入口' in page.locator('#service-cards').inner_text())
  ...
  page.locator('#chat-settings-open').click();page.locator('#cloud-access-code').fill('incorrect-code');page.locator('#cloud-login button').click();expect(page.locator('#cloud-access-note')).to_contain_text('体验码不匹配');check('bad access code receives a recovery instruction')
  ...
  page.locator('#message').fill('上海共享充电宝怎么租');page.locator('#send').click();expect(page.locator('.assistant-answer')).to_contain_text('租借柜');expect(page.locator('.answer-services')).to_contain_text('官方指引');check('automatic RAG answers without selected source',page.evaluate('TravelApp.getSourceIds().length')==0)
  ...
  page.locator('[data-csat=up]').click();page.locator('[data-resolution=solved]').click();expect(page.locator('[data-resolution=solved]')).to_have_attribute('aria-pressed','true');check('CSAT and solved feedback are independent')
  ...
  page.screenshot(path=str(OUT/'operations-quality-desktop.png'),full_page=True)
  ...
  page.locator('.skill-release summary').click();page.locator('[data-skill-publish]').click();expect(page.locator('#ops-status')).to_contain_text('已发布');check('admin publishes accepted exact skill version')
  page.locator('.skill-release summary').click();page.locator('[data-skill-rollback]').click();expect(page.locator('#ops-status')).to_contain_text('恢复上一版');check('published skill has working rollback')
  page.screenshot(path=str(OUT/'operations-harness-desktop.png'),full_page=True)
  ...
  page.screenshot(path=str(OUT/'operations-retrieval-desktop.png'),full_page=True)
  ...
  page.set_viewport_size({'width':390,'height':844});page.locator('[data-service-kind=hotel]').click();check('mobile layout has no horizontal overflow',page.evaluate('document.documentElement.scrollWidth<=innerWidth+1'));page.screenshot(path=str(OUT/'services-mobile.png'),full_page=True)
...
(OUT/'harness-browser-report.json').write_text(json.dumps({'checks':checks,'errors':errors,'scope':'Local server/auth/persistent store with controlled model HTTP; no real paid call'},ensure_ascii=False,indent=2),encoding='utf-8')
